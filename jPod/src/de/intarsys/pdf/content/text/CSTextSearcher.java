...
public class CSTextSearcher extends CSCharacterParser {
	...
	protected String getFloatingContextString() {
		String a = new String(floatingContext, floatingContextCurrent,
				floatingContext.length - floatingContextCurrent);
	...
	protected void onCharacterFound(PDGlyphs glyphs, Rectangle2D rect) {
		...
		if (Math.abs(dX) < maxDX) {
			if (Math.abs(dY) >= maxDY) {
				Rectangle2D spaceRect = new Rectangle2D.Float((int) lastStartX,
						(int) lastStartY, 0, 0);
				check(' ', spaceRect);
			}
		} else {
